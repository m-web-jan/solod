export function sendRequest(category, table){
    fetch(`http://localhost:3000/${table}?categoryId=${category}`)
    .then((response) => response.json())
    .then((data) => {

      displayCards(data, table);
    })
    .catch((error) => console.error("Error fetching data:", error));
}

export function displayCards(data, table) {
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      productWeight1.innerText = table=="drinks"? "объем":"Вес";
      let productWeight2 = document.createElement("p");
      productWeight2.innerText = `${data[i].weight}${table=="drinks"? "мл.":"г."}`;
